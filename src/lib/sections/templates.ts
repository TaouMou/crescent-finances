...
/**
 * Salary → Save / Invest / Give, expenses take the rest. From each salary, set
 * aside fixed shares for Savings (15%), Investment (10%) and Charity (5%); the
 * remainder (70%) covers living expenses. Always balances to 100%.
 */
function buildSalarySplit(): PlanBuild {
  const groupId = crypto.randomUUID();
  const group: SectionGroup = {
    id: groupId,
    name: 'Monthly plan',
  ...
  const sections: Section[] = [
    {
      ...
      groupId,
      order: 0,
      calc: { type: 'percentage', of: { kind: 'income' }, percent: 15 }
    },
    {
      id: crypto.randomUUID(),
      name: 'Investment',
      ...
      groupId,
      order: 1,
      calc: { type: 'percentage', of: { kind: 'income' }, percent: 10 }
    },
    {
      id: crypto.randomUUID(),
      name: 'Charity',
      ...
      groupId,
      order: 2,
      calc: { type: 'percentage', of: { kind: 'income' }, percent: 5 }
    },
    {
      id: crypto.randomUUID(),
      name: 'Expenses',
...
/**
 * Track one spending category: a single tracked bucket with a monthly cap, so
 * you see actual spending against planned. After applying, open it to choose
 * which category/accounts it watches.
 */
function buildTrackCategory(): PlanBuild {
  const groupId = crypto.randomUUID();
  const group: SectionGroup = {
    id: groupId,
  ...
  const sections: Section[] = [
    {
      ...
      groupId,
      order: 0,
      // Empty filter to start; user narrows it to a category after applying.
      // ~€300/month cap as a starting point.
      calc: { type: 'filterSum', filter: {}, planned: 30_000 }
    }
  ];
  group.sectionIds = sections.map((s) => s.id);
  return { sectionGroups: [group], sections };
...
export const planTemplates: PlanTemplate[] = [
  ...
  {
    id: 'salary-split',
    title: 'Salary → Save / Invest / Give',
    description:
      'From each salary, set aside Savings 15%, Investment 10% and Charity 5%; the rest covers expenses. See how your month went at a glance.',
    build: buildSalarySplit
  },
  {
    id: 'track-category',
    title: 'Track one spending category',
    description:
      'Watch a single category (e.g. Groceries) against a monthly cap, so you learn how actual vs planned works. Open it to pick the category.',
    build: buildTrackCategory,
